...
public class TestApp {
	...
	public TestApp(MembershipService membershipService, PaymentService paymentService) {
		this.membershipService = membershipService;
		this.paymentService = paymentService;

		menu = new MenuItem[] {
				new MenuItem("결제 시작", this::handlePayment),
				new MenuItem("프로그램 종료", this::exit)
		};
	}
	...
	public void run() {
		while (true) {
			printMenu();
			int ix = getMenuChoice();
			if (ix >= 0 && ix < menu.length) {
				menu[ix].getCommand().run();
			} else {
				System.out.println("유효한 메뉴 번호를 선택하세요.");
			}
		}
	}
	...
	private MembershipVO getOrRegisterMember(String phone) {
	...
	private void handlePayment() {
		...
		MembershipVO member = getOrRegisterMember(phone);
		...
		if (usePointAnswer.equalsIgnoreCase("y")) {
			...
			if (usePoint > 5000) { //숫자대신 결제할 금액이 들어갈 예정
				System.out.println("현재 결제할 금액보다 많이 사용할 수 없습니다.");
				return;
			}

			membershipService.addPoint(phone, -usePoint);
			System.out.printf("%d포인트를 사용했습니다.\n", usePoint);
		}

		int pointToAdd = (5000 - usePoint) / 10;  //5000 대신 결제할 금액이 들어갈 예정
		System.out.printf("%d원 만큼 차감하여 %d원 결제하였습니다.\n", usePoint, 5000 - usePoint);
	...
	public static void main(String[] args) {
		...
		TestApp app = new TestApp(membershipService, paymentService);
		app.run();
	}
}
